...
\begin{frame}{This session}
  ...
  \vskip0.5em
  \begin{itemize}
    \item What spatial transcriptomics is, the data, the libraries, the standard methods,
      and where they break.
    \item Two notebooks: an introduction to the data, libraries and workflow, then spatial
      methods and the open challenges.
    \item The higher-order / topological project (next part).
  \end{itemize}
  \vskip0.3em
...
\begin{frame}{Why spatial?}
  ...
  \vskip0.5em
  Concretely, ST hands you a \textbf{geometry} (points in a plane) with a high-dimensional
  signal (expression) attached, plus rich structure to exploit.
  \end{column}
...
\begin{frame}{Cell-cell communication: the standard logic}
  ...
  \begin{tikzpicture}[scale=1]
    ...
    \node[box] (B) at (4.2,0) {B\\(receptor $R$)};
